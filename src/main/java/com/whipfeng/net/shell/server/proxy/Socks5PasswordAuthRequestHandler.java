...
public class Socks5PasswordAuthRequestHandler extends SimpleChannelInboundHandler<DefaultSocks5PasswordAuthRequest> {
    ...
    protected void messageReceived(final ChannelHandlerContext ctx, final DefaultSocks5PasswordAuthRequest msg) throws Exception {
        ...
        if (msg.decoderResult().isSuccess()) {
            Future<Boolean> authFuture = ctx.channel().eventLoop().submit(new Callable<Boolean>() {
                @Override
                public Boolean call() throws Exception {
                    return passwordAuth.auth(msg.username(), msg.password());
                }
            });
            authFuture.addListener(new GenericFutureListener<Future<Boolean>>() {
                @Override
                public void operationComplete(Future<Boolean> authFuture) throws Exception {
                    if (authFuture.isSuccess() && authFuture.get()) {
